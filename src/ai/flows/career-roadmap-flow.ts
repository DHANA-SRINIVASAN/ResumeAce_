...
const RoadmapStepSchema = z.object({
  ...
  description: z.string().describe("A detailed description of what this step entails (e.g., skills to learn, type of experience to gain)."),
  estimatedTimeline: z.string().describe("An estimated timeline to complete this step (e.g., '3-6 months', '1 year')."),
  resources: z.array(z.string()).optional().describe("A list of suggested resources (e.g., specific course types, books, platforms)."),
});
...
const CareerRoadmapOutputSchema = z.object({
  ...
  steps: z.array(RoadmapStepSchema).describe("A list of actionable steps to reach the target role."),
...
Please generate the roadmap with the following sections:
...
2.  Steps: Create a list of 3-5 actionable steps. Each step should include:
    ...
    *   description: Detailed actions, skills to acquire, or experiences to gain.
    *   estimatedTimeline: A realistic timeline for the step (e.g., "3-6 months", "Ongoing").
    *   resources: (Optional) Suggest types of resources like "Online courses on AI/ML", "Networking events", "Open-source contributions".
3.  Potential Certifications: List 2-3 relevant certifications that would be beneficial for the target role.
4.  Project Ideas: Suggest 2-3 project ideas that would help build a strong portfolio for the target role. These should be practical.
